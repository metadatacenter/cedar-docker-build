...
This directory contains the Dockerfile and image-specific files for `cedar-microservice`. It is an
implementation unit of the managed CEDAR image build, not a standalone deployment path.

## Build Contract

This is an internal build base rather than a runtime service. It extends `cedar-java` with the
filesystem and launch contract shared by the fifteen `cedar-server-*` images.
